...
class UNet(nn.Module):
    def __init__(self, input_data, window_size, embedding_dim, n_channels, n_classes, n_blocks, start_channels,
                 end_from_h, residual, no_skip, batch_size=1, use_lcn=False, use_tte=False, feature_from=None):
        ...
        if not use_tte:
            ...
            for _ in range(self.end_from_h):  # TODO (check): was model size fixed to max h?
                self.ste.add_module('pos{}'.format(_), LearnablePosition(embedding_dim, x_size, y_size))
        
        self.inc = nn.Sequential()
        self.inc.add_module('inc', BasicConv(embedding_dim * window_size + (1 if use_tte else 0), start_channels, residual=residual))
        ...
        if use_lcn:
            self.outc.add_module('out_lcn', LCN2DLayer(in_channels=start_channels, out_channels=n_classes, x_size=x_size, y_size=y_size))
        else:
    ...
    def forward(self, x, target_time=None):
        ...
        """
        These parts are intentional conflicts
        """
        # Down blocks
