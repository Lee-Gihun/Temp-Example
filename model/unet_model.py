...
class UNet(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Conv2d(
            in_channels=self.input_channels, out_channels=embedding_dim, kernel_size=1
        )
        self.end_from_h = end_from_h
        ...
        if not use_tte:
            ...
            for _ in range(
                self.end_from_h
            ):  # TODO (check): was model size fixed to max h?
                self.ste.add_module(
                    "pos{}".format(_), LearnablePosition(embedding_dim, x_size, y_size)
                )
        ...
        self.inc.add_module(
            "inc",
            BasicConv(
                embedding_dim * window_size + (1 if use_tte else 0),
                start_channels,
                residual=residual,
            ),
        )
        ...
        self.bridge.add_module(
            "bridge_conv",
            BasicConv(bridge_channels, bridge_channels, residual=residual),
        )

        # Create up blocks
        self.up = nn.ModuleList([])
        skip = [
            (i not in no_skip) for i in range(n_blocks + 1)
        ]  # Enable/disable skip connections
        for i in range(n_blocks, 0, -1):
            cur_in_ch = start_channels << i
            self.up.append(
                Up(cur_in_ch, cur_in_ch // 2, residual=residual, skip=skip[i])
            )
        ...
        if feature_from is not None:
            ...
            if feature_from == "bridge":
                in_channels = bridge_channels
            else:
                in_channels = start_channels
            out_channels = embedding_dim
            self.projection.add_module("conv1", nn.Conv2d(in_channels, in_channels, 1))
            self.projection.add_module("relu", nn.ReLU())
            self.projection.add_module("conv2", nn.Conv2d(in_channels, out_channels, 1))
    ...
    def forward(self, x, target_time=None):
        ...
        for i in range(self.window_size):
            embedding_lst.append(
                self.embedding(
                    x[:, i * self.input_channels : (i + 1) * self.input_channels, :, :]
                )
            )
        ...
        if target_time is not None:
            ...
            if self.use_tte:  # Use TTE
                B, C, H, W = x.shape
                last_channel = (
                    torch.tensor([[[[((target_h + 8) % 24 + 1) / 24] * W] * H]] * B)
                    .cuda()
                    .float()
                )
                ...
                for i, idx in enumerate(
                    range(target_h - self.window_size + 1, target_h + 1)
                ):
                    x[
