...
#####################################################################
from .axial_attention import AxialAttention as axial_attn
from .axial_attention import AxialPositionalEmbedding as axial_PE
...
class MetNet(nn.Module):
    def __init__(self, input_data, window_size, num_cls, in_channels, start_dim, pred_hour, center_crop=False, center=None, feature_from=None):
        ...
        self.spatial_aggregator = nn.Sequential(
                AxialBlock(2*start_dim, 2*start_dim, groups=16, width=4*start_dim, kernel_size=(self.h // 2, self.w // 2)),
                AxialBlock(2*start_dim, 2*start_dim, groups=16, width=4*start_dim, kernel_size=(self.h // 2, self.w // 2)),
...
#                 nn.Upsample(size=(self.h + 2, self.w + 2), mode='bilinear', align_corners=True),
        ...
        if feature_from is not None:
            ...
            if feature_from == 'convlstm':
                in_channels = start_dim * 2
            else:
                in_channels = start_dim
            out_channels = start_dim // 8 # or add a new argument for it
    ...
    def forward(self, x, gk2a=None, target_lead_t=60, current_t=None):
        # TODO : Add elevation
        ...
        # upsampling
        # TODO : check upsampling layer.
        out = self.upsampling(out)
        if self.feature_from == 'up':
            feature = out.clone()
            feature = self.projection(feature)
            
        out = self.out_layer(out)
        
        if self.feature_from is not None:
            return feature, out
        else:
            return None, out
